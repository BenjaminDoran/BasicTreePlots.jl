...
function Makie.plot!(plt::TreePlot)
	...
	map!(
		...
		branchsegments = BasicTreePlots.makesegments(
			nodepoints,
			tree;
			branchstyle,
			resolution,
	...
	if plt.branchcolor[] isa Union{AbstractVector, AbstractRange} &&
	   length(plt.branchcolor[]) != length(plt.branchsegments[])
		error("length of branchcolor ($(length(plt.branchcolor[]))) must match number nodes in tree ($(length(plt.branchsegments[])))")
	end

	if plt.branchwidth[] isa Union{AbstractVector, AbstractRange} &&
	   length(plt.branchwidth[]) != length(plt.branchsegments[])
		error("length of branchwidth ($(length(plt.branchwidth[]))) must match number nodes in tree ($(length(plt.branchsegments[])))")
	end
...
function Makie.plot!(plt::TreeLabels)
	map!(plt.attributes,
		[:nodepoints, :nodelabels, :depth, :labelalign, :labelrotation, :labeloffset, :transform_func],
	...
	Makie.lines!(plt, plt.attributes, plt.guides_points;
...
function Makie.plot!(plt::TreeCladeLabel)
	inputs = [
		:nodepoints, :nodelabels,
		:lineoffset, :linepadding, :lineresolution,
		:labelrotation, :transform_func,
	]
	map!(
		plt.attributes,
		inputs,
	...
	map!(plt.attributes,
		...
		clade_regions =
			map(nodes) do node
				...
				clade_region = tf isa Polar ? Polygon(reverse.(clade_region)) : Polygon(clade_region)
				return clade_region
			end
