## from https://github.com/MakieOrg/Makie.jl/blob/master/docs/figure_block.jl
...
Documenter.Selectors.matcher(::Type{FigureBlocks}, node, page, doc) =
    Documenter.iscode(node, r"^@figure")
...
function Documenter.Selectors.runner(::Type{FigureBlocks}, node, page, doc)
    ...
    if blockname != ""
        ...
        prev = node.previous
        while prev !== nothing
            if prev.element isa Documenter.MultiOutput &&
               prev.element.codeblock.info == "@example $blockname"
                is_continued = true
                break
            end
            prev = prev.previous
    ...
    kwargs = Dict(
        map(kwargs) do expr
            if !(expr isa Expr) &&
               ...
               expr.args[2] isa Union{String,Number,Symbol}
    ...
    return MarkdownAST.insert_after!(
        node,
...
function transform_figure_code(
    code::String;
    id::String,
    page::String,
    pagetitle::String,
